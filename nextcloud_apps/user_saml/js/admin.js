...
$(function() {
	...
	if(type === 'environment-variable') {
		...
		$('#user-saml-settings .button').addClass('hidden');
		$('#user-saml-ldap').removeClass('hidden');
	}
	...
	$('#user-saml-ldap input[type="text"], #user-saml-ldap textarea').change(function(e) {
		var el = $(this);
		$.when(el.focusout()).then(function() {
			var key = $(this).attr('name');
			OCA.User_SAML.Admin.setSamlConfigValue('ldap', key, $(this).val());
		});
		if (e.keyCode === 13) {
			var key = $(this).attr('name');
			OCA.User_SAML.Admin.setSamlConfigValue('ldap', key, $(this).val());
