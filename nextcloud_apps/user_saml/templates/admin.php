...
<form id="user-saml" class="section" action="#" method="post" data-type="<?php p($_['type']) ?>">
	...
	<div id="user-saml-settings">
		...
		<div class="warning hidden" id="user-saml-warning-admin-user">
			<?php p(
				$l->t(
					'Make sure to configure an administrative user that can access the instance via SSO. Logging-in with admin account or your regular %s account won\'t be possible anymore unless you enable the local account authentication below.',
					[
						$theme->getEntity()
					]
				)
		...
		<div id="user-saml-general">
			...
			<!-- FIXME: Add "Disable timeout from SAML" switch (checked by default)-->
               ...
               <div id="user-saml-ldap">
                       <h3><?php p($l->t('LDAP Data for eduPersonPrincipalName resolving')) ?></h3>
		       <?php print_unescaped($l->t('URL:')) ?>
                       <p><input name="eppn_url" value="<?php p(\OC::$server->getConfig()->getAppValue('user_saml', 'ldap-eppn_url', '')) ?>" type="text" class="required" placeholder="<?php p($l->t('LDAP URL')) ?>"/></p>
		       <?php print_unescaped($l->t('Bind user:')) ?>
                       <p><input name="eppn_bind_user" value="<?php p(\OC::$server->getConfig()->getAppValue('user_saml', 'ldap-eppn_bind_user', '')) ?>" type="text" class="required" placeholder="<?php p($l->t('LDAP bind username')) ?>"/></p>
		       <?php print_unescaped($l->t('Bind user password:')) ?>
                       <p><input name="eppn_bind_pass" value="<?php p(\OC::$server->getConfig()->getAppValue('user_saml', 'ldap-eppn_bind_pass', '')) ?>" type="text" class="required" placeholder="<?php p($l->t('LDAP bind password')) ?>"/></p>
		       <?php print_unescaped($l->t('Search base:')) ?>
                       <p><input name="eppn_base" value="<?php p(\OC::$server->getConfig()->getAppValue('user_saml', 'ldap-eppn_base', '')) ?>" type="text" class="required" placeholder="<?php p($l->t('LDAP base')) ?>"/></p>
                       </p>
               </div>
