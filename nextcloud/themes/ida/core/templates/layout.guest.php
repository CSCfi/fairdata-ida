        ...
        <table style="max-width: 1280px">
            <tr>
                <?php if ((strpos($_SERVER['REQUEST_URI'], '/s/NOT_FOR_PUBLICATION_') !== false)
                          || (\OC::$server->getSystemConfig()->getValue('SSO_API') == '')
                          || (\OC::$server->getSystemConfig()->getValue('LOCAL_LOGIN') == true)) : ?>
                    <td rowspan="5">
                        <div id="ida-login" class="wrapper">
                            <div class="v-align">
                                ...
                                <?php elseif ((\OC::$server->getSystemConfig()->getValue('SSO_API') == '')
                                              || (\OC::$server->getSystemConfig()->getValue('LOCAL_LOGIN') == true)) : ?>
                                    <div class="local-login-form">
                                        <?php print_unescaped($_['content']); ?>
                                    </div>
                ...
                <?php if ((strpos($_SERVER['REQUEST_URI'], '/s/NOT_FOR_PUBLICATION_') == false) 
                    ...
                    <td align="right" style="text-align: right">
                        <a href="<?php p(\OC::$server->getSystemConfig()->getValue('SSO_API')) ?>/login?service=IDA&redirect_url=<?php p(\OC::$server->getSystemConfig()->getValue('IDA_HOME')) ?>">
                            <img src="/apps/ida/img/login.png" width="80px" style="padding-top: 45px" />
                        </a>
                    </td>
                <?php endif; ?>
            </tr>
