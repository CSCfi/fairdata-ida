...
<meta name="fdwe-scope" content="ACTIONS / FAILED">
...
<div id="app" class="ida">
            ...
            <table class="idaTable">
                <?php if (sizeof($_['actions']) > 0) : ?>
                    <tr>
                        ...
                        <th class="ida-timestamp"><?php p($l->t('Failed')) ?></th>
                    ...
                    <?php foreach ($_['actions'] as $action): ?>
                        <tr>
                            ...
                            <td><?php p((new \DateTime($action->getFailed()))->format('D, M j, Y, H:i') . ' UTC') ?></td>
                            <td><?php p($l->t($action->getAction())) ?></td>
                            <td><?php p($action->getProject()) ?></td>
                            <td class="ida-fixed"><?php p($action->getPathname()) ?></td>
                            <td class="ida-action-option"><a href="/apps/ida/retry/<?php p($action->getPid()) ?>"><?php p($l->t('RETRY')) ?></a></td>
                            <td class="ida-action-option">
                                <?php if (substr(\OC::$server->getUserSession()->getUser()->getUID(), 0, 4) === "PSO_"): ?>
                ...
                <?php else: ?>
                    <tr>
                        <td><?php p($l->t('No failed actions found.')) ?></td>
                    </tr>
                <?php endif; ?>
            </table>
        </div>
    ...
    <script nonce="<?php p(\OC::$server->getContentSecurityPolicyNonceManager()->getNonce()) ?>">
        document.title = "Failed Actions - IDA"; 
    </script>
</div>
