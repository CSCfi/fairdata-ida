...
(function () {
    ...
    function executeAction(fileInfo, action) {
        ...
        $.ajax({
            ...
            error: function (data) {
                ...
                if (action === 'freeze') {
                    OC.dialogs.alert(
                        t('ida', 'Unable to freeze the specified files:' + ' ' + response.responseJSON.message),
                        t('ida', 'Action Failed'),
                        function (result) {
                            window.location.reload(true);
                        },
                    ...
                    OC.dialogs.alert(
                        t('ida', 'Unable to unfreeze the specified files:' + ' ' + response.responseJSON.message),
                        t('ida', 'Action Failed'),
                        function (result) {
                            window.location.reload(true);
                        },
                    ...
                    OC.dialogs.alert(
                        t('ida', 'Unable to delete the specified files:' + ' ' + response.responseJSON.message),
                        t('ida', 'Action Failed'),
                        function (result) {
                            window.location.reload(true);
                        },
    ...
    /*
    function extractProjectName(pathname) {
    ...
    function stripRootFolder(pathname) {
        matches = pathname.match('^\/[^\/][^\/]*\/');
        if (matches.length > 0) {
    ...
    function extractBasename(pathname) {
        matches = pathname.match('[^\/][^\/]*$');
        if (matches.length > 0) {
            return matches[0];
        }
    ...
    function testIfRootProjectFolder(pathname) {
        return pathname.search('^/[^\/][^\/]*$') >= 0;
    }

    function testIfFrozen(pathname) {
        var project = extractProjectName(pathname);
                    ...
                    if (isFrozen) {
                        ...
                        if (isFolder) {
                            ...
                            $.ajax({
                                ...
                                cache: false,
                                //url: OC.generateUrl('/apps/ida/api/files/byProjectPathname/' + project + '?pathname=' + encodeURIComponent(pathname)),
